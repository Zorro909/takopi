...
from takopi.matrix.client import (
    DELETE_PRIORITY,
    EDIT_PRIORITY,
    SEND_PRIORITY,
    TYPING_PRIORITY,
...
from tests.matrix_fixtures import MATRIX_ROOM_ID, MATRIX_SENDER, MATRIX_USER_ID
...
def test_build_reply_content() -> None:
    ...
    from takopi.matrix.client import _build_reply_content
...
def test_build_edit_content() -> None:
    ...
    from takopi.matrix.client import _build_edit_content
...
class TestE2EEAutoTrust:
    ...
    @pytest.mark.anyio
    async def test_trust_room_devices_when_e2ee_unavailable(self) -> None:
        """trust_room_devices is a no-op when E2EE unavailable."""
        from takopi.matrix.availability import NioAvailability
        from takopi.matrix.client import MatrixClient
        ...
        client = MatrixClient(
            ...
            access_token="token",
            _nio_availability=availability,
        )
...
class TestReplyTextFetching:
    ...
    async def test_get_event_text_returns_body_from_event(self) -> None:
        ...
        mock_nio_client.room_get_event.assert_called_once_with("!room:matrix.org", "$event123")
    ...
    async def test_get_event_text_returns_none_on_error(self) -> None:
        ...
        mock_nio_client = MagicMock()
        mock_nio_client.room_get_event = AsyncMock(side_effect=RuntimeError("Network error"))
        client._nio_client = mock_nio_client
